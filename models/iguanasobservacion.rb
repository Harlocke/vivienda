class Iguanasobservacion < ActiveRecord::Base
  belongs_to :user
  belongs_to :iguana
  ...
  def after_create
    ...
    iguanaspersonas = Iguanaspersona.find_all_by_iguana_id(self.iguana_id)
    iguanaspersonas.each do |iguanaspersona|
      p = Personasobservacion.new
      p.user_id = self.user_id
      p.tipo_atencion = self.tipo_atencion
      p.observacion = "OBSERVACION REASENTAMIENTO - " + self.observacion
      p.persona_id = iguanaspersona.persona_id
