...
xml.Workbook({
  ...
  xml.Worksheet 'ss:Name' => 'Atencion - Valorizacion' do
    xml.Table do
      # Header
      # Header
      ...
      # Rows
      cadena = ""
      for valorizacionesobservacion in @valorizacionesobservaciones
        #Atencion al predio con identificacion de plano <> cuyo propietario(s) es(son) <> . <atencion>
        cadena = "ATENCIÓN AL PREDIO CON IDENTIFICACION DE PLANO NRO " + valorizacionesobservacion.valorizacion.nro_registro
        cadena = cadena + ", CUYO PROPIETARIO(S) ES(SON): "
        @valorizacionespersonas = Valorizacionespersona.find_all_by_valorizacion_id(valorizacionesobservacion.valorizacion_id);
        ...
        cadena = cadena + valorizacionesobservacion.observacion

        xml.Row do
            xml.Cell { xml.Data valorizacionesobservacion.created_at, 'ss:Type' => 'String' }
            xml.Cell { xml.Data valorizacionesobservacion.dtipoatencion, 'ss:Type' => 'String' }
            if (valorizacionesobservacion.user_id.to_s != "")
              nombreuser = User.find(valorizacionesobservacion.user_id)
